...
/// resultant address.
pub fn apollo(config: Config) -> Result<(), Box<dyn Error>> {
	...
	// let re = Regex::new(r"(?im:^aaaa)|(?im:aaaa$)|(?im:^ab0110)|(?im:ab0110$)").unwrap();
	// let re = Regex::new(r"(?im:^aaaa.+aaaa$)|(?im:^ab0110)|(?im:ab0110$)").unwrap();
	let re = Regex::new(r"(?im:^ab0110.+aaaa$)|(?im:^aaaa.+aaaa$)").unwrap();
		...
		(0..MAX_INCREMENTER)
			...
			.for_each(|salt_incremented_segment| {
				...
				if re.is_match(&address) {
					...
					println!("full salt: {} => 0x{}", full_salt, address);
					// write the salt and address to the file
				}
			});
	}
}
			...
			.for_each(|salt_incremented_segment| {
				...
				if total > 2 {
					...
					if reward_amount != ZERO_REWARD {
						...
						// display the salt and the address.
						let output = format!("{} => {} => {}", full_salt, checksum_address, reward_amount);
						println!("{}", &output);
